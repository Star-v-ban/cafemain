...
const toggle = () => {
    if (productStore.products.length >= 1 && productStore.products.every(product => product.eatBy.length > 0)) {
        router.push({ name: 'result' });
    } else {
        alert("Выберите для кого каждый продукт был выбран");
    }
};
</script>
                    ...
                    <v-list-item v-for="(product, index) in productStore.products" :key="product.id">
                        <v-card>
                            <v-list-item class="d-flex flex-column justify-start align-start ma-5 ">
                                <v-list-item-title class="d-flex flex-row justify-start align-center ml-5 ">
